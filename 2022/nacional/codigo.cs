...
async Task BaixarGarra() {
    ...
    while(Bot.GetComponent<Servomotor>("Garra2").Angle > 50) {
//(AnguloInicial-Bot.GetComponent<Servomotor>("Garra2").Angle).ToString());
        Bot.GetComponent<Servomotor>("Garra2").Apply(500, -2000);
        await Time.Delay(16);
    }
...
async Task LevantarGarra() {
    ...
    while(Bot.GetComponent<Servomotor>("Garra2").Angle < AnguloInicial+10) {
        Bot.GetComponent<Servomotor>("Garra2").Apply(500, 320);
        await Time.Delay(16);      
    }
    while(Bot.GetComponent<Servomotor>("Garra2").Angle < AnguloInicial+29) {
        Bot.GetComponent<Servomotor>("Garra2").Apply(500, 350);
        await Time.Delay(16);      
    }
    Bot.GetComponent<Servomotor>("Garra2").Locked = true;
...
async Task Entregar() {
    Bot.GetComponent<Servomotor>("Garra2").Locked = false;
    Bot.GetComponent<Servomotor>("Garra2").Apply(500, -1200);
    await Time.Delay(2000);
    await AbrirGarra();
    Bot.GetComponent<Servomotor>("Garra2").Locked = true;
    await Time.Delay(1500);
    Bot.GetComponent<Servomotor>("Garra2").Locked = false;
    Bot.GetComponent<Servomotor>("Garra2").Apply(500, 300);
    await Time.Delay(300);
    Bot.GetComponent<Servomotor>("Garra2").Locked = true;
}
async Task EntregarKit() {
    Bot.GetComponent<Servomotor>("Garra2").Locked = false;
    Bot.GetComponent<Servomotor>("Garra2").Apply(500, 1200);
...
async Task ProcurarSaida() {
    IO.Print("ta na hora de ir embora");
}
async Task VarrerDireita() {
    await Destravar();
    int TimerRedzone = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-TimerRedzone < 15000) {
        await Frente(200);
        await Time.Delay(16);
        // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog).ToString());
        if(Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog < 16 && Bot.GetComponent<UltrasonicSensor>("UltraDireita").Digital==true) {
            await Frente(200);
            ...
            while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Inicial < 11860) {
                await Frente(150);
                await Time.Delay(16);
            }
            await FecharGarra();
            ...
            await Destravar();
            await Alinhar();
            int Tempo_Parede = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
            while(Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog > 5 && (int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Parede < 11860)
            {
                IO.Print((DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Parede).ToString());
            await Frente(200);
            await Time.Delay(16);
            }
            await Girar(90,0);
            ...
            await Destravar();
            await Alinhar();
            TimerRedzone = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
        }
    }
}
async Task VarrerEsquerda() {
    await Destravar();
    int TimerRedzone = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-TimerRedzone < 15000) {
        await Frente(200);
        await Time.Delay(16);
        // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog).ToString());
        if(Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog < 16 && Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Digital==true) {
            await Frente(200);
            await Time.Delay(575);
            await Girar(-90, 0);
            await Alinhar();
            ...
            await LevantarGarra();
            await Time.Delay(500);
            int DistanciaTriangulo = (int)Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog;
            await Destravar();
            await Frente(-100);
            await Time.Delay(1000);
            await Girar(-90, 0);
            ...
            await Girar(-135,1);
            await Parar();
                IO.Print("eu girei exatamente 135 graus");
            await BaixarGarra();
            await Time.Delay(500);
            await Destravar();
            await Alinhar();
...
async Task VarrerDireitaParcial() {
    await Destravar();
    while(Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog >= 22 || Bot.GetComponent<UltrasonicSensor>("UltraDireita").Digital==false) {
        await Frente(200);
        await Time.Delay(16);
    }
   // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog).ToString());
    await Frente(200);
    await Time.Delay(575);
    await Girar(90, 0);
    await Alinhar();
    ...
    await Destravar();
    int Tempo_Inicial = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Inicial < 18480) {
        await Frente(150);
        await Time.Delay(16);
    }
    await FecharGarra();
    ...
    await Alinhar();
    int Tempo_Parede = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while(Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog > 5 && (int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Parede < 12000)
    {
    await Frente(200);
    await Time.Delay(16);
    }
...
async Task VarrerEsquerdaParcial() {
    await Destravar();
    while(Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog >= 22 || Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Digital==false) {
        await Frente(200);
        await Time.Delay(16);
    }
   // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog).ToString());
...
async Task VarrerDiagonal(int DistanciaParede) {
    await Destravar();
    while(Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog >= 16 || Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Digital==false) {
        await Frente(200);
        await Time.Delay(16);
    }
   // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog).ToString());
    ...
    await Destravar();
    int Tempo_Inicial = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-Tempo_Inicial < DistanciaParede) {
        await Frente(150);
        await Time.Delay(16);
    }
    await FecharGarra();
...
async Task Redzone() {
    ...
    int TempoInicio = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
    while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-TempoInicio < 2000) {
        Direita = Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog;
        await AndarReto(200);
        await Time.Delay(16);
        // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraDireita").Digital).ToString());
        if(Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog-Direita > 0.05 && Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog-Direita < 0.2) {
            TrianguloDireita++;
            IO.Print(Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog.ToString());
        }
        if(Bot.GetComponent<UltrasonicSensor>("UltraDireita").Digital==false || Bot.GetComponent<UltrasonicSensor>("UltraDireita").Analog > 25) {
            RedzoneDeitada++;
    ...
    if (TrianguloDireita > 5 && RedzoneDeitada < 5) { //de pé
        ...
        await Alinhar();
        double Esquerda = 0;
        int TrianguloEsquerda = 0;
        TempoInicio = (int)DateTimeOffset.Now.ToUnixTimeMilliseconds();
        while((int)DateTimeOffset.Now.ToUnixTimeMilliseconds()-TempoInicio < 2000) {
            Esquerda = Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog;
            await Frente(200);
            await Time.Delay(16);
            // IO.Print((Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Digital).ToString());
            if(Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog-Esquerda > 0.05 && Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog-Esquerda < 0.2) {
                TrianguloEsquerda++;
                IO.Print(Bot.GetComponent<UltrasonicSensor>("UltraEsquerda").Analog.ToString());
            }
        }
        ...
        if (TrianguloEsquerda > 5 && RedzoneDeitada < 5) {
            ...
            await VarrerEsquerda();
            await ProcurarSaida();
        } else if (RedzoneDeitada < 5) {
            IO.Print("Redzone de pe caso 3");
            await VarrerDiagonal(18500);
            await VarrerEsquerda();
            await ProcurarSaida();
        } else {
            IO.Print("Redzone deitada caso 3");
            await VarrerDiagonal(11860);
            await VarrerEsquerda();
            await ProcurarSaida();
        }
    }
...
async Task DoisPretos() {
    ...
    if(Bot.GetComponent<ColorSensor>("CorDireita").Analog.Green > Bot.GetComponent<ColorSensor>("CorDireita").Analog.Red + 30 || Bot.GetComponent<ColorSensor>("CorEsquerda").Analog.Green > Bot.GetComponent<ColorSensor>("CorEsquerda").Analog.Red + 30) {
        await Verde();
    } else if(Bot.GetComponent<ColorSensor>("CorDireita").Analog.Blue > Bot.GetComponent<ColorSensor>("CorDireita").Analog.Red +7 && Bot.GetComponent<ColorSensor>("CorDireita").Analog.Blue  > Bot.GetComponent<ColorSensor>("CorDireita").Analog.Green && Bot.GetComponent<ColorSensor>("CorDireita").Analog.Red  < 120) {
        await Redzone();
    } else {
        int ValorGiro;
        if(Luz(1) > Luz(2)) {
...
async Task Desvio() {
    if(Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog < 1.5 && Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog > 0 && (Bot.Inclination > 355 || Bot.Inclination < 10)) {
        await Time.Delay(50);
        if(Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog < 1.5 && Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog > 0 && (Bot.Inclination > 355 || Bot.Inclination < 10)) {
            while(Bot.GetComponent<UltrasonicSensor>("UltraFrente").Analog < 2.1) {
                await Frente(-50);
                await Time.Delay(50);
            }
            await Girar(60, 1);
